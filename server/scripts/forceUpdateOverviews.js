...
// Force update all product overviews
...
function forceUpdateOverviews() {
  const overviewData = getProductOverviews();
  const updateStmt = db.prepare(`UPDATE products SET overview = ? WHERE name = ?`);
  let updatedCount = 0;

  for (const [productName, data] of Object.entries(overviewData)) {
    const result = updateStmt.run(JSON.stringify(data), productName);
    if (result.changes > 0) {
